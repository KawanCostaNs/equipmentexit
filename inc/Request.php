...
use Notepad;
use Log;     
use \Location; // Adicionado para buscar localizações
...
class Request extends CommonDBTM {
    ...
    // *** MUDANÇA (Gemini): Trocado 'self-service' por 'all' para permitir acesso de aprovadores (a segurança é feita manualmente em cada página) ***
    ...
    // *** ATUALIZADO (Dinâmico): Lista de Lojas do GLPI (com o filtro WHERE removido para evitar erros) ***
    static function getStoreList() {
        global $DB;
        $locations = array(); 
        $location_table = Location::getTable(); // Pega o nome da tabela (glpi_locations)

        // Prepara a consulta para buscar todas as localizações
        $query = $DB->request(array(
            'SELECT' => 'name',
            'FROM'   => $location_table,
            // 'WHERE'  => ... FILTRO REMOVIDO PARA EVITAR ERRO DE COLUNA DESCONHECIDA ...
            'ORDER'  => 'name ASC'
        ));

        // Transforma o resultado da consulta em um array simples de nomes
    ...
    function getItems() {
        ...
        $query = $DB->request(array( 
            ...
            'WHERE'  => array(self::$fk_field => $this->getID()) 
        ));
        foreach ($query as $item_data) {
            $items[] = $item_data;
        }
        return $items;
    }
    ...
    function rawSearchOptions() {
        $options = array(); 
        $options[] = array('id'   => 'common', 'name' => __('Características')); 
        $options[] = array('id' => 1, 'table' => self::getTable(), 'field' => 'id', 'name'  => __('ID'), 'datatype' => 'number', 'display'  => true,); 
        $options[] = array('id' => 2, 'table' => self::getTable(), 'field' => 'status', 'name'  => __('Status'), 'datatype' => 'specific', 'display'  => true,); 
        $options[] = array('id' => 3, 'table' => self::getTable(), 'field' => 'id', 'name'  => __('Item(ns)', 'equipmentexit'), 'datatype'  => 'specific', 'display'   => true,); 
        $options[] = array( 
            'id'    => 4, 'table' => 'glpi_users', 'field' => 'name', 'name'  => __('Solicitante', 'equipmentexit'), 'datatype' => 'dropdown', 'display'  => true,
            'joinparams' => array('jointype' => 'inner', 'on' => array('table' => self::getTable(), 'field' => 'users_id_requester', 'eqfield' => 'id')) 
        );
        $options[] = array('id' => 5, 'table' => self::getTable(), 'field' => 'date_request', 'name'  => __('Data da Solicitação', 'equipmentexit'), 'datatype' => 'datetime', 'display'  => true,); 
        $options[] = array('id' => 8, 'table' => self::getTable(), 'field' => 'local_origem', 'name'  => __('Origem', 'equipmentexit'), 'datatype' => 'text', 'display'  => true,); 
        $options[] = array('id' => 80, 'table' => self::getTable(), 'field' => 'date_mod', 'name'  => __('Última atualização'), 'datatype' => 'datetime', 'display'  => false); 
        return $options;
    }
    ...
    static function canCreate($options = array()) { 
         global $DB;
        ...
        if ($result_check) {
             $count_row = $DB->fetchAssoc($result_check);
            if ($count_row !== false && isset($count_row['cpt'])) {
                $requester_count = (int)$count_row['cpt'];
            }
        }
...
__('-- Selecione uma loja --') . "</option>";
    ...
    function showForm($ID, array $options = array()) { 
        ...
        $existing_items = ($ID > 0) ? $this->getItems() : array(); 

        echo "<div class='equipment-form-container'>";
        echo "<form method='post' action='" . Html::clean($options['target']) . "'>";
        ...
        // --- Pergunta 1: Local de Origem ---
        echo "<tr><td colspan='4' class='section-header-cell'>";
        echo "<h2 class='form-section-title'><i class='fas fa-map-marker-alt'></i> " . __('1. Local de Origem') .
 "</h2>";
        echo "</td></tr>";
        ...
        // --- Pergunta 2: Informações da Movimentação ---
        echo "<tr><td colspan='4' class='section-header-cell'>";
        ...
        // --- Pergunta 3: Identificação dos Equipamentos ---
        echo "<tr><td colspan='4' class='section-header-cell'>";
        ...
        // --- Pergunta 4: Detalhes Adicionais ---
        echo "<tr><td colspan='4' class='section-header-cell'>";
        ...
        echo "<td>";
        Html::showDateTimeField('date_exit_planned', array( 
        ...
        echo "<td>";
        Html::showDateTimeField('date_return_planned', array( 
        ...
        if ($ID > 0) {
             echo Html::hidden('users_id_requester', array('value' => $this->fields['users_id_requester'])); 
        }
        if ($ID == 0) {
            echo Html::hidden('date_request', array('value' => date('Y-m-d H:i:s'))); 
        }
    ...
    private function displayItemBoxScript() {
        ...
        $store_dropdown_html = addslashes(str_replace(array("\r", "\n"), '', $store_dropdown_html)); 
...
__('Nome do Equipamento', 'equipmentexit') . " <span class=\'required\'>*</span></label><input type=\'text\' name=\'equipamento_nome[]\' value=\'\' class=\'glpi_input\'></div><div class=\'form-field\'><label>" . __('Tipo do Equipamento', 'equipmentexit') . " <span class=\'required\'>*</span></label><input type=\'text\' name=\'equipamento_tipo[]\' value=\'\' class=\'glpi_input\'></div><div class=\'form-field\'><label>" . __('Quantidade', 'equipmentexit') . " <span class=\'required\'>*</span></label><input type=\'number\' name=\'quantidade[]\' value=\'1\' class=\'glpi_input\' style=\'max-width: 120px;\'></div><div class=\'form-field\'><label>" . __('Loja de Destino', 'equipmentexit') . " <span class=\'required\'>*</span></label>$store_dropdown_html</div></div></div>";
    ...
    function prepareInputForAdd($input) {
     ...
     function post_updateItem($history = true) {
         ...
         $DB->delete(self::$item_table, array(self::$fk_field => $this->getID())); 
         $this->saveItems($_POST); 
     }
    ...
    function showFormButtons($options = array()) { 
        ...
        if ($ID > 0) {
            echo Html::submit(__('Salvar'), array('name' => 'update')); 
            if (empty($this->fields["is_deleted"])) {
                echo Html::submit(__('Mover para a lixeira'), array('name' => 'delete')); 
            } else {
                echo Html::submit(__('Restaurar'), array('name' => 'restore')); 
                echo Html::submit(__('Excluir permanentemente'), array('name' => 'purge')); 
            }
        } else {
            echo Html::submit(__('Adicionar'), array('name' => 'add')); 
